...
import { doSaveMvc, doSearchMvc, doUpdateMvc } from '../Services/profileControllerF';
...
//const url = "http://localhost:3007/GProfile/add-G";
  ...
  const serverMsg = await doSaveMvc(formdata, { headers: { 'Content-Type': 'multipart/form-data' } })
  ...
  async function doSearch() {
    ...
    const servermesg = await doSearchMvc(obj);
  ...
  async function doUpdateRec() {
    ...
    const serverMsg = await doUpdateMvc(formdata, { headers: { 'Content-Type': 'multipart/form-data' } }) ;
    console.log(serverMsg)
            ...
            <div class="border-b border-gray-900/10 pb-12 relative mt-10 border-b border-slate-200 rounded-md grid  ">
              ...
              <div class="mt-10 grid  gap-x-6 gap-y-8 sm:grid-cols-6">
                ...
                <div class="sm:col-span-2 sm:col-start-1">
                  <label for="city" class="block text-sm font-medium leading-6 text-gray-900">Product Category</label>
                  <div class="mt-2">
                    <select name="category" value={obj.category} onChange={doUpdate} id="" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" >
